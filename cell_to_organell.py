...
from ImagingCytometryToolsIMC.object_pixel_extraction import get_pixel_values
from ImagingCytometryToolsIMC.object_pixel_extraction import get_connected_black_pixels
from ImagingCytometryToolsIMC.object_pixel_extraction import find_edge_pixels
from ImagingCytometryToolsIMC.object_pixel_extraction import pixel_sort_counterclockwise
...
# for oddly shaped cells or extreme qualety controll
def cell_to_organell_advanced(Cells, Cytoplasm, Nucleus, Nucleus_count,outline_cell_image_dir,outline_nucleus_image_dir):
    print('I am working :)')
    ...
    # create a data frame dictionary to store your data frames
    DataFrameDict_Full_cell = {elem: pd.DataFrame() for elem in
                               UniqueNames_Full_cell}  # creates a dictionary of all unique images
    DataFrameDict_Cytoplasm = {elem: pd.DataFrame() for elem in
                               UniqueNames_Cytoplasm}  # creates a dictionary of all unique images
    DataFrameDict_Nucleus = {elem: pd.DataFrame() for elem in
                             UniqueNames_Nucleus}  # creates a dictionary of all unique images

    # creates a data frame to store matched subcellular locations
    columns = ['ImageNumber', 'Location_Center_X', 'Location_Center_Y', 'AreaShape_MinFeretDiameter',
               'AreaShape_MaxFeretDiameter', 'Cell_outline', 'Nuclear_outline']  # list for essential values and markers
    for markers in get_markers_from_segmentation(Cells):  # get all markers
        columns.append(markers + '_Cell')  # creates a column for the full cell
        columns.append(markers + '_Cytoplasm')  # creates a column for the cytoplasm
        columns.append(markers + '_Nucleus')  # creates a column for the nucleus
    ...
    for paths, dirs, files in sd.walk(outline_cell_image_dir):  # goes throw all files and folders in given directory
        ...
        for file in os.listdir(paths):  # goes throw all files in a folder
            ...
            if filedir.endswith(".tiff"):  # returns all files that end with txt

                filename = os.path.basename(file)

                file_dir_list_cell.append(filedir)
                file_name_list_cell.append(filename)

    file_dir_list_nucleus = []
    file_name_list_nucleus = []
    for paths, dirs, files in sd.walk(outline_nucleus_image_dir):  # goes throw all files and folders in given directory
        ...
        for file in os.listdir(paths):  # goes throw all files in a folder
            ...
            if filedir.endswith(".tiff"):  # returns all files that end with txt
                ...
                file_name_list_nucleus.append(filename)
    ...
    for key, value in DataFrameDict_Full_cell.items():
        ...
        for file in file_name_list_cell:
            outline_name = re.split(r'(\d+)', file)

            if outline_name[1] == str(key):

                outline_image = get_pixel_values(file_dir_list_cell[key-1])

                print(key)
                print(outline_name[1])

                count_cell = -1  # counter
                for cell in position_cells:  # goes throw all cell positions

                    connected_black_pixels = get_connected_black_pixels(outline_image,round(cell[1]),round(cell[0]))
                    edge_pixels = find_edge_pixels(connected_black_pixels,outline_image)
                    pixel_sorted = pixel_sort_counterclockwise(edge_pixels)

                    if len(pixel_sorted) >=4:
                        cell_outline = shapely.geometry.Polygon(pixel_sorted) # creates the smallest possible cell diameter

                        print(cell_outline.area)
            ...
            for cytoplasm in position_cytoplasm:  # goes throw all cytoplasm positions
                ...
                if cytoplasm_position.within(cell_shape) == True:
                    ...
                    if len(nucleus_list) == Nucleus_count:
                        ...
                        for marker in get_markers_from_segmentation(Cells):
                            MeanIntensity_Cell = DataFrameDict_Full_cell[key]['Intensity_' + marker].to_list()[
                                count_cell]
                            MeanIntensity_Cytoplasm = DataFrameDict_Cytoplasm[key]['Intensity_' + marker].to_list()[
                                count_cytoplasm]
                            MeanIntensity_Nucleus = DataFrameDict_Nucleus[key]['Intensity_' + marker].to_list()[
                                nucleus_list[0]]
