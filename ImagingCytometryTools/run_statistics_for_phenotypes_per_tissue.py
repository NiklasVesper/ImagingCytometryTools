...
def run_statistics_for_phenotypes_per_tissue(directory, filestring, df_column, output_folder, cell_types, percent_of = ['All'], add_mixed_cells = False, select_neighbors = [False], split_by_image = 'yes'):
                    ...
                    if filename in meta_data_list_1:
                        ...
                        if split_by_image == 'yes':
                            ...
                            for number in UniqueImageNumber:
                                ...
                                Cells_on_image['Meta_data_category'] = '1'

                                df_meta_data_1_column_list.append(filename_string + '_image_' + str(number))  # appends the file name

                                phenotype_df = Cells_on_image[df_column]  # selects the analysis column

                                df_meta_data_1 = pd.concat([df_meta_data_1, phenotype_df], axis=1, ignore_index=True)

                        if split_by_image == 'no':
                    ...
                    if filename in meta_data_list_2:
                        ...
                        if split_by_image == 'yes':
                            ...
                            for number in UniqueImageNumber:
                                ...
                                Cells_on_image['Meta_data_category'] = '2'

                                df_meta_data_2_column_list.append(filename_string + '_image_' + str(number))  # appends the file name

                                phenotype_df = Cells_on_image[df_column]  # selects the analysis column

                                df_meta_data_2 = pd.concat([df_meta_data_2, phenotype_df], axis=1, ignore_index=True)
    ...
    if percent_of[0] == 'lineage' and add_mixed_cells == True:
        ...
        for cell_state in state_list[:-len(analysis_values)]:
            ...
            if 0.005 < p < 0.05:
                axs.text(1.3, max_plot * 0.8, '*', fontsize=12, color='black')
            elif 0.0005 < p < 0.005:
                axs.text(1.3, max_plot * 0.8, '**', fontsize=12, color='black')
