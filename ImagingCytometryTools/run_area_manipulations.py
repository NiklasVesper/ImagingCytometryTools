...
from ImagingCytometryTools.area_manipulation import select_areas

'''
Runs the selection of  cells intersecting with specially chosen areas.
'''

def run_select_areas(directory, filestring, new_folder_name, selected_areas):
        ...
        for file in os.listdir(paths):  # goes throw all files in a folder
            ...
            if filedir.endswith(".csv"):  # returns all files that end with .csv
                ...
                if filestring in filename_string:  # returns all files that contain the provided file string
                    ...
                    export_file_path = filedir_ISO + '/' + filename_string[:-4] + '_selected_by_' + str(selected_areas) + '.csv'  # creates a export file path
                    ...
                    if os.path.isfile(export_file_path) == True:  # checks if the new file directory already exists
                        ...
                        splited_area = select_areas(file, selected_areas)
                        splited_area.to_csv(export_file_path)